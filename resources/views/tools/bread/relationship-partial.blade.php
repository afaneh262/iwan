...
<div class="row row-dd row-dd-relationship">
    <div class="col-xs-2">
        <h4><i class="iwan-heart"></i><strong>{{ $relationship->display_name }}</strong></h4>
        <div class="handler iwan-handle"></div>
        <strong>{{ __('iwan::database.type') }}:</strong> <span>{{ __('iwan::database.relationship.relationship') }}</span>
        <div class="handler iwan-handle"></div>
    ...
    <div class="col-xs-2">
        ...
        <label for="field_browse_{{ $relationship['field'] }}"> {{ __('iwan::database.relationship.browse') }}</label><br>
        ...
        <label for="field_read_{{ $relationship['field'] }}"> {{ __('iwan::database.relationship.read') }}</label><br>
        ...
        <label for="field_edit_{{ $relationship['field'] }}"> {{ __('iwan::database.relationship.edit') }}</label><br>
        ...
        <label for="field_add_{{ $relationship['field'] }}"> {{ __('iwan::database.relationship.add') }}</label><br>
        ...
        <label for="field_delete_{{ $relationship['field'] }}"> {{ __('iwan::database.relationship.delete') }}</label><br>
    </div>
    <div class="col-xs-2">
        <p>{{ __('iwan::database.relationship.relationship') }}</p>
    </div>
    <div class="col-xs-2">
    ...
    <div class="col-xs-4">
        <div class="iwan-relationship-details-btn">
            <i class="iwan-angle-down"></i><i class="iwan-angle-up"></i> 
            <span class="open_text">{{ __('iwan::database.relationship.open') }}</span>
            <span class="close_text">{{ __('iwan::database.relationship.close') }}</span>
            {{ __('iwan::database.relationship.relationship_details') }}
        </div>
    </div>
    <div class="col-md-12 iwan-relationship-details">
        <a href="{{ route('iwan.bread.delete_relationship', $relationship['id']) }}" class="delete_relationship"><i class="iwan-trash"></i> {{ __('iwan::database.relationship.delete') }}</a>
        <div class="relationship_details_content">
            ...
            <span><input type="text" class="form-control" name="relationship_model_{{ $relationship['field'] }}" placeholder="{{ __('iwan::database.relationship.namespace') }}" value="@if(isset($relationshipDetails->model)){{ $relationshipDetails->model }}@endif"></span>
        </div>
        ...
        <div class="relationship_details_content margin_top">
            <label>{{ __('iwan::database.relationship.display_the') }} <span class="label_table_name"></span></label>
            ...
            <label class="relationship_key" style="@if($relationshipDetails->type == 'belongsTo' || $relationshipDetails->type == 'belongsToMany'){{ 'display:block' }}@endif">{{ __('iwan::database.relationship.store_the') }} <span class="label_table_name"></span></label>
            ...
            @isset($relationshipDetails->taggable)
                <label class="relationship_taggable" style="@if($relationshipDetails->type == 'belongsToMany'){{ 'display:block' }}@endif">
                    {{__('iwan::database.relationship.allow_tagging')}}
                </label>
                <span class="relationship_taggable" style="@if($relationshipDetails->type == 'belongsToMany'){{ 'display:block' }}@endif">
                    <input type="checkbox" name="relationship_taggable_{{ $relationship['field'] }}" class="toggleswitch" data-on="{{ __('iwan::generic.yes') }}" data-off="{{ __('iwan::generic.no') }}" {{$relationshipDetails->taggable == 'on' ? 'checked' : ''}}>
                </span>
            @endisset
        </div>
    </div>
