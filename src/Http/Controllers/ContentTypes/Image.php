    ...
    public function handle()
    {
        if ($this->request->hasFile($this->row->field)) {
            ...
            if ($this->is_animated_gif($file)) {
                Storage::disk(config('iwan.storage.disk'))->put($fullPath, file_get_contents($file), 'public');
                $fullPathStatic = $path.$filename.'-static.'.$file->getClientOriginalExtension();
                Storage::disk(config('iwan.storage.disk'))->put($fullPathStatic, (string) $image, 'public');
            } else {
                Storage::disk(config('iwan.storage.disk'))->put($fullPath, (string) $image, 'public');
            }
            ...
            if (isset($this->options->thumbnails)) {
                foreach ($this->options->thumbnails as $thumbnails) {
                    ...
                    Storage::disk(config('iwan.storage.disk'))->put(
