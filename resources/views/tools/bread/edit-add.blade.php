...
@section('page_title', __('iwan::bread.edit_bread_for_table', ['table' => (isset($dataType->id) ? @$dataType->name : $table)]))

@section('page_header')
    <div class="page-title">
        <i class="iwan-data"></i>
        {{ __('iwan::bread.edit_bread_for_table', ['table' => (isset($dataType->id) ? @$dataType->name : $table)]) }}
    </div>
    @php
    ...
    @endphp
    @include('iwan::multilingual.language-selector')
@stop


@section('content')
    <div class="page-content container-fluid" id="iwanBreadEditAdd">
        <div class="row">
            <div class="col-md-12">

                <form action="@if(isset($dataType->id)){{ route('iwan.bread.update', $dataType->id) }}@else{{ route('iwan.bread.store') }}@endif"
                ...
                @endif
                    ...
                    <div class="panel panel-primary panel-bordered">
                        ...
                        <div class="panel-heading">
                            <h3 class="panel-title panel-icon"><i class="iwan-bread"></i> {{ ucfirst($table) }} {{ __('iwan::bread.bread_info') }}</h3>
                            <div class="panel-actions">
                                <a class="panel-action iwan-angle-up" data-toggle="panel-collapse" aria-hidden="true"></a>
                            </div>
                        </div>
                        ...
                        <div class="panel-body">
                            <div class="row clearfix">
                                <div class="col-md-6 form-group">
                                    <label for="name">{{ __('iwan::database.table_name') }}</label>
                            ...
                            <div class="row clearfix">
                                <div class="col-md-6 form-group">
                                    <label for="display_name_singular">{{ __('iwan::bread.display_name_singular') }}</label>
                                    ...
                                    <input type="text" class="form-control"
                                           ...
                                           placeholder="{{ __('iwan::bread.display_name_singular') }}"
                                           value="@if(isset($dataType->display_name_singular)){{ $dataType->display_name_singular }}@else{{ $display_name }}@endif">
                                </div>
                                <div class="col-md-6 form-group">
                                    <label for="display_name_plural">{{ __('iwan::bread.display_name_plural') }}</label>
                                    @if($isModelTranslatable)
                                        @include('iwan::multilingual.input-hidden', [
                                            'isModelTranslatable' => true,
                                    ...
                                    <input type="text" class="form-control"
                                           ...
                                           placeholder="{{ __('iwan::bread.display_name_plural') }}"
                                           value="@if(isset($dataType->display_name_plural)){{ $dataType->display_name_plural }}@else{{ $display_name_plural }}@endif">
                                </div>
                            </div>
                            <div class="row clearfix">
                                <div class="col-md-6 form-group">
                                    <label for="slug">{{ __('iwan::bread.url_slug') }}</label>
                                    <input type="text" class="form-control" name="slug" placeholder="{{ __('iwan::bread.url_slug_ph') }}"
                                           value="@if(isset($dataType->slug)){{ $dataType->slug }}@else{{ $slug }}@endif">
                                </div>
                                <div class="col-md-6 form-group">
                                    <label for="icon">{{ __('iwan::bread.icon_hint') }} <a
                                                href="{{ route('iwan.compass.index', [], false) }}#fonts"
                                                target="_blank">{{ __('iwan::bread.icon_hint2') }}</a></label>
                                    <input type="text" class="form-control" name="icon"
                                           placeholder="{{ __('iwan::bread.icon_class') }}"
                                           value="@if(isset($dataType->icon)){{ $dataType->icon }}@endif">
                                </div>
                            </div>
                            <div class="row clearfix">
                                <div class="col-md-6 form-group">
                                    <label for="model_name">{{ __('iwan::bread.model_name') }}</label>
                                    <span class="iwan-question"
                                        aria-hidden="true"
                                        data-toggle="tooltip"
                                        data-placement="right"
                                        title="{{ __('iwan::bread.model_name_ph') }}"></span>
                                    <input type="text" class="form-control" name="model_name" placeholder="{{ __('iwan::bread.model_class') }}"
                                           value="@if(isset($dataType->model_name)){{ $dataType->model_name }}@else{{ $model_name }}@endif">
                                </div>
                                <div class="col-md-6 form-group">
                                    <label for="controller">{{ __('iwan::bread.controller_name') }}</label>
                                    <span class="iwan-question"
                                        aria-hidden="true"
                                        data-toggle="tooltip"
                                        data-placement="right"
                                        title="{{ __('iwan::bread.controller_name_hint') }}"></span>
                                    <input type="text" class="form-control" name="controller" placeholder="{{ __('iwan::bread.controller_name') }}"
                                           value="@if(isset($dataType->controller)){{ $dataType->controller }}@endif">
                                </div>
                            </div>
                            <div class="row clearfix">
                                <div class="col-md-6 form-group">
                                    <label for="policy_name">{{ __('iwan::bread.policy_name') }}</label>
                                    <span class="iwan-question"
                                          aria-hidden="true"
                                          data-toggle="tooltip"
                                          data-placement="right"
                                          title="{{ __('iwan::bread.policy_name_ph') }}"></span>
                                    <input type="text" class="form-control" name="policy_name" placeholder="{{ __('iwan::bread.policy_class') }}"
                                           value="@if(isset($dataType->policy_name)){{ $dataType->policy_name }}@endif">
                                </div>
                                <div class="col-md-3 form-group">
                                    <label for="generate_permissions">{{ __('iwan::bread.generate_permissions') }}</label><br>
                                    ...
                                    <input type="checkbox" name="generate_permissions" class="toggleswitch" data-on="{{ __('iwan::generic.yes') }}" data-off="{{ __('iwan::generic.no') }}"
                                           @if($checked) checked @endif>
                                </div>
                                <div class="col-md-3 form-group">
                                    <label for="server_side">{{ __('iwan::bread.server_pagination') }}</label><br>
                                    <?php $checked = (isset($dataType->server_side) && $dataType->server_side == 1) ? true : (isset($server_side) && $server_side) ? true : false; ?>
                                    <input type="checkbox" name="server_side" class="toggleswitch" data-on="{{ __('iwan::generic.yes') }}" data-off="{{ __('iwan::generic.no') }}"
                                           @if($checked) checked @endif>
                                </div>
                            </div>
                            <div class="row clearfix">
                                <div class="col-md-6 form-group">
                                    <label for="order_column">{{ __('iwan::bread.order_column') }}</label>
                                    <span class="iwan-question"
                                          aria-hidden="true"
                                          data-toggle="tooltip"
                                          data-placement="right"
                                          title="{{ __('iwan::bread.order_column_ph') }}"></span>
                                    <select name="order_column" class="select2 form-control">
                                          <option value="">-- {{ __('iwan::generic.none') }} --</option>
                                ...
                                <div class="col-md-6 form-group">
                                    <label for="order_display_column">{{ __('iwan::bread.order_ident_column') }}</label>
                                    <span class="iwan-question"
                                          aria-hidden="true"
                                          data-toggle="tooltip"
                                          data-placement="right"
                                          title="{{ __('iwan::bread.order_ident_column_ph') }}"></span>
                            ...
                            <div class="form-group">
                                <label for="description">{{ __('iwan::bread.description') }}</label>
                                <textarea class="form-control" name="description"
                                          placeholder="{{ __('iwan::bread.description') }}"
                    ...
                    <div class="panel panel-primary panel-bordered">
                        <div class="panel-heading">
                            <h3 class="panel-title panel-icon"><i class="iwan-window-list"></i> {{ __('iwan::bread.edit_rows', ['table' => $table]) }}:</h3>
                            <div class="panel-actions">
                                <a class="panel-action iwan-angle-up" data-toggle="panel-collapse" aria-hidden="true"></a>
                            </div>
                        </div>
                        ...
                        <div class="panel-body">
                            <div class="row fake-table-hd">
                                <div class="col-xs-2">{{ __('iwan::database.field') }}</div>
                                <div class="col-xs-2">{{ __('iwan::database.visibility') }}</div>
                                <div class="col-xs-2">{{ __('iwan::database.input_type') }}</div>
                                <div class="col-xs-2">{{ __('iwan::bread.display_name') }}</div>
                                <div class="col-xs-4">{{ __('iwan::database.optional_details') }}</div>
                            </div>
                            ...
                            @foreach($fieldOptions as $data)
                                ...
                                @if(isset($dataType->id))
                                    <?php $dataRow = Afaneh262\Iwan\Models\DataRow::where('data_type_id', '=',
                                ...
                                <div class="row row-dd">
                                    <div class="col-xs-2">
                                        ...
                                        <strong>{{ __('iwan::database.type') }}:</strong> <span>{{ $data['type'] }}</span><br/>
                                        <strong>{{ __('iwan::database.key') }}:</strong> <span>{{ $data['key'] }}</span><br/>
                                        <strong>{{ __('iwan::generic.required') }}:</strong>
                                        @if($data['null'] == "NO")
                                            <span>{{ __('iwan::generic.yes') }}</span>
                                            <input type="hidden" value="1" name="field_required_{{ $data['field'] }}"
                                                   checked="checked">
                                        @else
                                            <span>{{ __('iwan::generic.no') }}</span>
                                            <input type="hidden" value="0" name="field_required_{{ $data['field'] }}">
                                        @endif
                                        <div class="handler iwan-handle"></div>
                                    ...
                                    <div class="col-xs-2">
                                        <input type="checkbox"
                                               ...
                                               @elseif(!isset($dataRow->browse))
                                                    {{ 'checked="checked"' }}
                                               @endif>
                                        <label for="field_browse_{{ $data['field'] }}">{{ __('iwan::generic.browse') }}</label><br/>
                                        <input type="checkbox"
                                        ...
                                        <label for="field_read_{{ $data['field'] }}">{{ __('iwan::generic.read') }}</label><br/>
                                        <input type="checkbox"
                                        ...
                                        <label for="field_edit_{{ $data['field'] }}">{{ __('iwan::generic.edit') }}</label><br/>
                                        <input type="checkbox"
                                            ...
                                            <label for="field_add_{{ $data['field'] }}">{{ __('iwan::generic.add') }}</label><br/>
                                        <input type="checkbox"
                            ...
                            @if(isset($dataTypeRelationships))
                                @foreach($dataTypeRelationships as $relationship)
                                    @include('iwan::tools.bread.relationship-partial', $relationship)
                                @endforeach
                            @endif
                        ...
                        <div class="panel-footer">
                             <div class="btn btn-new-relationship"><i class="iwan-heart"></i> <span>
                             {{ __('iwan::database.relationship.create') }}</span></div>
                        </div>
                    </div><!-- .panel -->

                    <button type="submit" class="btn pull-right btn-primary">{{ __('iwan::generic.submit') }}</button>
...
@include('iwan::tools.bread.relationship-new-modal')
...
@section('javascript')
    ...
    <script>
            ...
            $('textarea[data-editor]').each(function () {
                ...
                textarea.closest('form').on('submit', function (ev) {
                    if (window.invalidEditors.length) {
                        ...
                        toastr.error('{{ __('iwan::json.invalid_message') }}', '{{ __('iwan::json.validation_errors') }}', {"preventDuplicates": true, "preventOpenDuplicates": true});
                    } else {
       ...
       $(function () {
            ...
            $('.iwan-relationship-details-btn').click(function(){
                $(this).toggleClass('open');
                if($(this).hasClass('open')){
                    $(this).parent().parent().find('.iwan-relationship-details').slideDown();
                } else {
                    $(this).parent().parent().find('.iwan-relationship-details').slideUp();
                }
            });
        ...
        function populateRowsFromTable(dropdown){
            ...
            if(tbl.length != 0){
                $.get('{{ route('iwan.database.index') }}/' + tbl, function(data){
                    $(dropdown).empty();
