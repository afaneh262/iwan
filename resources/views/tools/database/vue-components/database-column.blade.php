...
<tr class="newTableRow">
    ...
    <td>
        <select :value="index.type" @change="onIndexTypeChange"
                ...
                class="form-control">
            <option value=""></option>
            <option value="INDEX">{{ __('iwan::database.index') }}</option>
            <option value="UNIQUE">{{ __('iwan::database.unique') }}</option>
            <option value="PRIMARY">{{ __('iwan::database.primary') }}</option>
        </select>
        <small v-if="column.composite" v-once>{{ __('iwan::database.composite_warning') }}</small>
    </td>
    ...
    <td>
        <div class="btn btn-danger delete-row" @click="deleteColumn"><i class="iwan-trash"></i></div>
    </td>
...
@include('iwan::tools.database.vue-components.database-types')
@include('iwan::tools.database.vue-components.database-column-default')
    ...
    Vue.component('database-column', {
        ...
        methods: {
            ...
            onIndexTypeChange(event) {
                if (this.column.name == '') {
                    return toastr.error("{{ __('iwan::database.name_warning') }}");
                }
