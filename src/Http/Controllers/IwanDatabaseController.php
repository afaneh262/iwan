...
use Afaneh262\Iwan\Database\DatabaseUpdater;
use Afaneh262\Iwan\Database\Schema\Column;
use Afaneh262\Iwan\Database\Schema\Identifier;
...
use Afaneh262\Iwan\Events\TableAdded;
use Afaneh262\Iwan\Events\TableDeleted;
use Afaneh262\Iwan\Events\TableUpdated;
use Afaneh262\Iwan\Facades\Iwan;
use Afaneh262\Iwan\Models\DataType;

class IwanDatabaseController extends Controller
{
    public function index()
    {
        Iwan::canOrFail('browse_database');
        ...
        return Iwan::view('iwan::tools.database.index')->with(compact('dataTypes', 'tables'));
    }
    ...
    public function create()
    {
        ...
        return Iwan::view('iwan::tools.database.edit-add', compact('db'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if (isset($request->create_model) && $request->create_model == 'on') {
                ...
                Artisan::call('iwan:make:model', $params);
            ...
            return redirect()
               ...
               ->with($this->alertSuccess(__('iwan::database.success_create_table', ['table' => $table->name])));
        } catch (Exception $e) {
    ...
    public function edit($table)
    {
        ...
        if (!SchemaManager::tableExists($table)) {
            return redirect()
                ->route('iwan.database.index')
                ->with($this->alertError(__('iwan::database.edit_table_not_exist')));
        }
    ...
    protected function prepareDbManager($action, $table = '')
    {
        ...
        if ($action == 'update') {
            ...
            $db->formAction = route('iwan.database.update', $table);
        } else {
            ...
            $db->formAction = route('iwan.database.store');
        }
    ...
    public function show($table)
    {
        ...
        $model_name = Iwan::model('DataType')->where('name', $table)->pluck('model_name')->first();
        if (isset($model_name)) {
    ...
    public function destroy($table)
    {
        ...
        try {
            ...
            return redirect()
                ->route('iwan.database.index')
                ->with($this->alertSuccess(__('iwan::database.success_delete_table', ['table' => $table])));
        } catch (Exception $e) {
