@extends('iwan::master')
@if($db->action == 'update')
    @section('page_title', __('iwan::database.editing_table', ['table' => $db->table->name]))
@else
    @section('page_title', __('iwan::database.create_new_table'))
@endif
...
@section('page_header')
    <h1 class="page-title">
        <i class="iwan-data"></i>
        @if($db->action == 'update')
            {{ __('iwan::database.editing_table', ['table' => $db->table->name]) }}
        @else
            {{ __('iwan::database.create_new_table') }}
        @endif
    </h1>
@stop
...
@section('javascript')
    @include('iwan::tools.database.vue-components.database-table-editor')
