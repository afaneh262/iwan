...
@section('page_title', __('iwan::generic.menu_builder'))

@section('page_header')
    <h1 class="page-title">
        <i class="iwan-list"></i>{{ __('iwan::generic.menu_builder') }} ({{ $menu->name }})
        <div class="btn btn-success add_item"><i class="iwan-plus"></i> {{ __('iwan::menu_builder.new_menu_item') }}</div>
    </h1>
            ...
            <div class="col-md-12">
                <div class="panel panel-bordered">
                    <div class="panel-heading">
                        <p class="panel-title" style="color:#777">{{ __('iwan::menu_builder.drag_drop_info') }}</p>
                    </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title"><i class="iwan-trash"></i> {{ __('iwan::menu_builder.delete_item_question') }}</h4>
                </div>
                <div class="modal-footer">
                    <form action="{{ route('iwan.menus.item.destroy', ['menu' => $menu->id, 'id' => '__id']) }}"
                          id="delete_form"
                          method="POST">
                        {{ method_field("DELETE") }}
                        {{ csrf_field() }}
                        <input type="submit" class="btn btn-danger pull-right delete-confirm"
                               value="{{ __('iwan::menu_builder.delete_item_confirm') }}">
                    </form>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="{{ __('iwan::generic.close') }}"><span
                                aria-hidden="true">&times;</span></button>
                    <h4 id="m_hd_add" class="modal-title hidden"><i class="iwan-plus"></i> {{ __('iwan::menu_builder.create_new_item') }}</h4>
                    <h4 id="m_hd_edit" class="modal-title hidden"><i class="iwan-edit"></i> {{ __('iwan::menu_builder.edit_item') }}</h4>
                </div>
                <form action="" id="m_form" method="POST"
                      data-action-add="{{ route('iwan.menus.item.add', ['menu' => $menu->id]) }}"
                      data-action-update="{{ route('iwan.menus.item.update', ['menu' => $menu->id]) }}">

                    <input id="m_form_method" type="hidden" name="_method" value="POST">
                    {{ csrf_field() }}
                    <div class="modal-body">
                        @include('iwan::multilingual.language-selector')
                        <label for="name">{{ __('iwan::menu_builder.item_title') }}</label>
                        @include('iwan::multilingual.input-hidden', ['_field_name' => 'title', '_field_trans' => ''])
                        <input type="text" class="form-control" id="m_title" name="title" placeholder="{{ __('iwan::generic.title') }}"><br>
                        <label for="type">{{ __('iwan::menu_builder.link_type') }}</label>
                        <select id="m_link_type" class="form-control" name="type">
                            <option value="url" selected="selected">{{ __('iwan::menu_builder.static_url') }}</option>
                            <option value="route">{{ __('iwan::menu_builder.dynamic_route') }}</option>
                        </select><br>
                        <div id="m_url_type">
                            <label for="url">{{ __('iwan::menu_builder.url') }}</label>
                            <input type="text" class="form-control" id="m_url" name="url" placeholder="{{ __('iwan::generic.url') }}"><br>
                        </div>
                        <div id="m_route_type">
                            <label for="route">{{ __('iwan::menu_builder.item_route') }}</label>
                            <input type="text" class="form-control" id="m_route" name="route" placeholder="{{ __('iwan::generic.route') }}"><br>
                            <label for="parameters">{{ __('iwan::menu_builder.route_parameter') }}</label>
                        ...
                        <label for="icon_class">{{ __('iwan::menu_builder.icon_class') }} <a
                                    href="{{ route('iwan.compass.index', [], false) }}#fonts"
                                    target="_blank">{!! __('iwan::menu_builder.icon_class2') !!}</label>
                        <input type="text" class="form-control" id="m_icon_class" name="icon_class"
                               placeholder="{{ __('iwan::menu_builder.icon_class_ph') }}"><br>
                        <label for="color">{{ __('iwan::menu_builder.color') }}</label>
                        <input type="color" class="form-control" id="m_color" name="color"
                               placeholder="{{ __('iwan::menu_builder.color_ph') }}"><br>
                        <label for="target">{{ __('iwan::menu_builder.open_in') }}</label>
                        <select id="m_target" class="form-control" name="target">
                            <option value="_self" selected="selected">{{ __('iwan::menu_builder.open_same') }}</option>
                            <option value="_blank">{{ __('iwan::menu_builder.open_new') }}</option>
                        </select>
                        <input type="hidden" name="menu_id" value="{{ $menu->id }}">
                        <input type="hidden" name="id" id="m_id" value="">
                    </div>
                    <div class="modal-footer">
                        <input type="submit" class="btn btn-success pull-right delete-confirm__" value="{{ __('iwan::generic.update') }}">
                        <button type="button" class="btn btn-default pull-right" data-dismiss="modal">{{ __('iwan::generic.cancel') }}</button>
                    </div>
                </form>
            </div><!-- /.modal-content -->
    ...
    <script>
        $(document).ready(function () {
            ...
            $('.add_item').click(function() {
                ...
                $m_form.find("input[type=submit]").val('{{ __('iwan::generic.add') }}');
            ...
            $('.item_actions').on('click', '.edit', function (e) {
                $m_form.find("input[type=submit]").val('{{ __('iwan::generic.update') }}');
            ...
            $('.dd').on('change', function (e) {
                $.post('{{ route('iwan.menus.order',['menu' => $menu->id]) }}', {
                    order: JSON.stringify($('.dd').nestable('serialize')),
                    _token: '{{ csrf_token() }}'
                }, function (data) {
                    toastr.success("{{ __('iwan::menu_builder.updated_order') }}");
                });
            });
        });
    </script>
