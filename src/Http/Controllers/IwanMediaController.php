...
class IwanMediaController extends Controller
{
    /** @var string */
    private $filesystem;
    ...
    {
        $this->filesystem = config('iwan.storage.disk');
    }

    public function index()
    {
        // Check permission
        Iwan::canOrFail('browse_media');

        return Iwan::view('iwan::media.index');
    }
    ...
    public function listFiles(Request $request)
    {
        ...
        $entries = Iwan::model('Media')::when(!$is_admin, function ($query) {
            return $query->where('created_by', user()->id);
        })->when($types, function ($query) use ($types) {
            return $query->whereIn('type', explode(',', $types));
        })->when($hosted, function ($query) use ($hosted) {
            return $query->where('hosted', $hosted);
        })->when($name_query, function ($query) use ($name_query) {
            return $query->where('original_name', 'like', '%' . $name_query . '%');
        })
            ->orderBy('id', 'desc')
            ->paginate(30);
    ...
    public function new_folder(Request $request)
    {
        ...
        if (Storage::disk($this->filesystem)->exists($new_folder)) {
            $error = __('iwan::media.folder_exists_already');
        } elseif (Storage::disk($this->filesystem)->makeDirectory($new_folder)) {
            $success = true;
        } else {
            $error = __('iwan::media.error_creating_dir');
        }
    ...
    public function delete_file_folder(Request $request)
    {
        ...
        if ($type == 'folder') {
            if (!Storage::disk($this->filesystem)->deleteDirectory($fileFolder)) {
                $error = __('iwan::media.error_deleting_folder');
                $success = false;
            }
        } elseif (!Storage::disk($this->filesystem)->delete($fileFolder)) {
            $error = __('iwan::media.error_deleting_file');
            $success = false;
        }
    ...
    public function move_file(Request $request)
    {
        ...
        if (!file_exists($destination)) {
            if (Storage::disk($this->filesystem)->move($source, $destination)) {
                ...
                $error = __('iwan::media.error_moving');
            }
        } else {
            $error = __('iwan::media.error_already_exists');
        }
    ...
    public function rename_file(Request $request)
    {
        ...
        if (!Storage::disk($this->filesystem)->exists("{$location}/{$newFilename}")) {
            ...
            $error = __('iwan::media.error_may_exist');
        }
    ...
    public function upload2(Request $request)
    {
        if ($request->hasFile('file')) {
            $validator = Validator::make($request->all(), [
                'file' => 'required|mimetypes:video/mp4,video/avi,video/mpeg,video/quicktime,image/jpeg,image/png,image/jpg,image/gif,image/svg,audio/mp4,audio/m4a,audio/x-m4a,audio/mp3,audio/mpeg3,audio/mpg,audio/x-mp3,audio/x-mpeg,audio/x-mpeg,audio/mpga,audio/x-mpg,audio/mpeg|max:211222222222222222',
                'resource' => 'required',
            ]);

            if ($validator->passes()) {
                $file = $request->file('file');
                $media = upload_media_resource($file, input('resource'));
        ...
        if ($validator->passes()) {
            ...
            if (strpos($path, "youtube")) {
                $fileType = 'video';
            } else if (strpos($path, "vimeo")) {
                $fileType = 'video';
            } else if (strpos($path, "soundcloud")) {
                $fileType = 'audio';
            } else {
                return response()->json(['error' => 'Unsupported website']);
            }
            ...
            $media = Iwan::model('Media')::create(array(
                ...
                'cloud_thumbnail' => input('cloud_thumbnail'),
                'cloud_host' => input('cloud_host'),
                'original_name' => $path,
            ));
    ...
    public function upload(Request $request)
    {
        try {
            ...
            $success = true;
            $message = __('iwan::media.success_uploaded_file');
    ...
    private function getFiles($dir)
    {
        ...
        foreach ($storageItems as $item) {
            if ($item['type'] == 'dir') {
                ...
                if (empty(pathinfo($item['path'], PATHINFO_FILENAME)) && !config('iwan.hidden_files')) {
                    continue;
                }
                $files[] = [
                    'name'          => $item['basename'],
    ...
    public function remove(Request $request)
    {
        try {
            ...
            // Check permission
            Iwan::canOrFail('delete_'.$dataType->name);
            ...
            if (!isset($data->{$field})) {
                throw new Exception(__('iwan::generic.field_does_not_exist'), 400);
            }

            // Check if valid json
            if (is_null(@json_decode($data->{$field}))) {
                throw new Exception(__('iwan::json.invalid'), 500);
            }
            ...
            if (!array_key_exists($image, $fieldData)) {
                throw new Exception(__('iwan::media.image_does_not_exist'), 400);
            }
            ...
            return response()->json([
               'data' => [
                   'status'  => 200,
                   'message' => __('iwan::media.image_removed'),
               ],
            ]);
        } catch (Exception $e) {
            $code = 500;
            $message = __('iwan::generic.internal_error');
    ...
    public function crop(Request $request)
    {
        ...
        try {
            ...
            $success = true;
            $message = __('iwan::media.success_crop_image');
        } catch (Exception $e) {
            $success = false;
            $message = $e->getMessage();
        }
