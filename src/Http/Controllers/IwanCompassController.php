...
class IwanCompassController extends Controller
{
    protected $request;
    ...
    public function index(Request $request)
    {
        ...
        if ($this->request->input('download')) {
            ...
            return $this->redirect($this->request->url().'?logs=true')->with([
                'message'    => __('iwan::compass.commands.delete_success').' '.base64_decode($this->request->input('del')),
                'alert-type' => 'success',
                ]);
            ...
            return $this->redirect($this->request->url().'?logs=true')->with([
                'message'    => __('iwan::compass.commands.delete_all_success'),
                'alert-type' => 'success',
                ]);
        }
        ...
        return view('iwan::compass.index', compact('logs', 'files', 'current_file', 'active_tab', 'commands', 'artisan_output'))->with($message);
    }
