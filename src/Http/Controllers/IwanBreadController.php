...
use Afaneh262\Iwan\Database\Schema\Column;
use Afaneh262\Iwan\Database\Schema\SchemaManager;
use Afaneh262\Iwan\Database\Schema\Table;
use Afaneh262\Iwan\Database\Types\Type;
use Afaneh262\Iwan\Events\BreadAdded;
use Afaneh262\Iwan\Events\BreadDeleted;
use Afaneh262\Iwan\Events\BreadUpdated;
use Afaneh262\Iwan\Facades\Iwan;
use Afaneh262\Iwan\Models\DataRow;
use Afaneh262\Iwan\Models\DataType;
use Afaneh262\Iwan\Models\Permission;

class IwanBreadController extends Controller
{
    public function index()
    {
        Iwan::canOrFail('browse_bread');

        $dataTypes = Iwan::model('DataType')->select('id', 'name', 'slug')->get()->keyBy('name')->toArray();
        ...
        return Iwan::view('iwan::tools.bread.index')->with(compact('dataTypes', 'tables'));
    }
    ...
    public function create(Request $request, $table)
    {
        ...
        return Iwan::view('iwan::tools.bread.edit-add', $data);
    }

    private function prepopulateBreadInfo($table)
    {
        $displayName = Str::singular(implode(' ', explode('_', Str::title($table))));
        $modelNamespace = config('iwan.models.namespace', app()->getNamespace());
    ...
    {
        try {
            $dataType = Iwan::model('DataType');
            $res = $dataType->updateDataType($request->all(), true);
            $data = $res
                ? $this->alertSuccess(__('iwan::bread.success_created_bread'))
                : $this->alertError(__('iwan::bread.error_creating_bread'));
            if ($res) {
                event(new BreadAdded($dataType, $data));
            }

            return redirect()->route('iwan.bread.index')->with($data);
        } catch (Exception $e) {
            return redirect()->route('iwan.bread.index')->with($this->alertException($e, 'Saving Failed'));
        }
    }
    ...
    public function edit($table)
    {
        ...
        $dataType = Iwan::model('DataType')->whereName($table)->first();

        $fieldOptions = SchemaManager::describeTable($dataType->name);

        $isModelTranslatable = is_bread_translatable($dataType);
        $tables = SchemaManager::listTableNames();
        $dataTypeRelationships = Iwan::model('DataRow')->where('data_type_id', '=', $dataType->id)->where('type', '=', 'relationship')->get();

        return Iwan::view('iwan::tools.bread.edit-add', compact('dataType', 'fieldOptions', 'isModelTranslatable', 'tables', 'dataTypeRelationships'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        /* @var \Afaneh262\Iwan\Models\DataType $dataType */
        try {
            $dataType = Iwan::model('DataType')->find($id);
            ...
            $res = $dataType->updateDataType($request->all(), true);
            $data = $res
                ? $this->alertSuccess(__('iwan::bread.success_update_bread', ['datatype' => $dataType->name]))
                : $this->alertError(__('iwan::bread.error_updating_bread'));
            if ($res) {
            ...
            return back()->with($this->alertException($e, __('iwan::generic.update_failed')));
        }
    }
    ...
    public function destroy($id)
    {
        ...
        $res = Iwan::model('DataType')->destroy($id);
        $data = $res
            ? $this->alertSuccess(__('iwan::bread.success_remove_bread', ['datatype' => $dataType->name]))
            : $this->alertError(__('iwan::bread.error_updating_bread'));
        if ($res) {
        ...
        if (!is_null($dataType)) {
            Iwan::model('Permission')->removeFrom($dataType->name);
        }
    ...
    public function addRelationship(Request $request)
    {
        ...
        try {
            ...
            $newRow->order = intval(Iwan::model('DataType')->find($request->data_type_id)->lastRow()->order) + 1;
    ...
    private function getRelationshipField($request)
    {
        ...
        $dataType = Iwan::model('DataType')->find($request->data_type_id);
        ...
        $existingRow = Iwan::model('DataRow')->where('field', '=', $relationshipField)->first();
        $index = 1;
