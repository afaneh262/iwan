...
class Translator implements ArrayAccess
{
    ...
    public function __construct(Model $model)
    {
        ...
        $this->model = $model;
        $this->locale = config('iwan.multilingual.default', 'en');
        $attributes = [];
    ...
    public function save()
    {
        ...
        foreach ($attributes as $key => $attribute) {
            if ($attribute['exists']) {
                ...
                $translation = IwanFacade::model('Translation')->where('table_name', $this->model->getTable())
            ...
            if (is_null($translation)) {
                $translation = IwanFacade::model('Translation');
            }
    ...
    protected function translateAttributeToOriginal($attribute)
    {
        $this->attributes[$attribute] = [
            ...
            'locale'   => config('iwan.multilingual.default', 'en'),
            'exists'   => true,
            'modified' => false,
        ];
    ...
    public function deleteTranslation($key)
    {
        ...
        IwanFacade::model('Translation')->where('table_name', $this->model->getTable())
            ->where('column_name', $key)
            ->where('foreign_key', $this->model->getKey())
            ->where('locale', $locale)
            ->delete();
    ...
    public function __call($method, array $arguments)
    {
        if (!$this->model->hasTranslatorMethod($method)) {
            throw new \Exception('Call to undefined method Afaneh262\Iwan\Translator::'.$method.'()');
        }
