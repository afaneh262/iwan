...
use Afaneh262\Iwan\Actions\DeleteAction;
use Afaneh262\Iwan\Actions\EditAction;
use Afaneh262\Iwan\Actions\ViewAction;
use Afaneh262\Iwan\Events\AlertsCollection;
use Afaneh262\Iwan\FormFields\After\HandlerInterface as AfterHandlerInterface;
use Afaneh262\Iwan\FormFields\HandlerInterface;
use Afaneh262\Iwan\Models\Category;
...
use Afaneh262\Iwan\Models\Translation;
use Afaneh262\Iwan\Models\User;
use Afaneh262\Iwan\Models\Media;
...
class Iwan
{
    ...
    {
        $widgetClasses = config('iwan.dashboard.widgets');
        $dimmers = Widget::group('iwan::dimmers');
    ...
    {
        if (!empty($file)) {
            return str_replace('\\', '/', Storage::disk(config('iwan.storage.disk'))->url($file));
        }
    ...
    {
        require __DIR__.'/../routes/iwan.php';
    }
    ...
    protected function findVersion()
    {
        ...
        if ($this->filesystem->exists(base_path('composer.lock'))) {
            ...
            // Loop through all the packages and get the version of iwan
            foreach ($file->packages as $package) {
                if ($package->name == 'afaneh262/iwan') {
    ...
    {
        if (!config('iwan.multilingual.enabled')) {
            return false;
        }
