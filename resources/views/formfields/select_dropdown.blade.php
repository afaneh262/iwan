@if(isset($options->relationship))
    ...
    @if( !method_exists( $dataType->model_name, camel_case($row->field) ) )
        <p class="label label-warning"><i class="iwan-warning"></i> {{ __('iwan::form.field_select_dd_relationship', ['method' => camel_case($row->field).'()', 'class' => $dataType->model_name]) }}</p>
    @endif
    ...
    @if( method_exists( $dataType->model_name, camel_case($row->field) ) )
        ...
        <select class="form-control select2" name="{{ $row->field }}">
            ...
            @if(isset($options->options))
                <optgroup label="{{ __('iwan::generic.custom') }}">
            ...
            <optgroup label="{{ __('iwan::database.relationship.relationship') }}">
